...
// 存在交换的情况,仅处理是否取消
        if ((inputItem != null) && (outputItem != null)) {
            //2个应该是同一个
            val slot = baseUI.getSlot(inputItem.first)
            if (slot == null) {
                if (baseUI.lockOnTop || baseUI.lockOnBottom) {
                    isCancelled = true
                }
                return
            } else if (slot is IOSlot) {
        ...
        // 仅输出
        if (outputItem != null) {
            val slot = baseUI.getSlot(outputItem.first)
            if (slot == null) {
        ...
        if (inputItem != null) {
            ...
            if (slot == null) {
                if (inputItem.first in 0 until baseUI.inventory.size && baseUI.lockOnTop)
                    setCancel = true
            } else if (slot is IOSlot && slot.input(slot, inputItem.second)) {
